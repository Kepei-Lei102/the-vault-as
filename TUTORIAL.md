...
That one is small and quick — clone it **first**, then start the vault, so the little one
finishes while the big one downloads. It may look almost empty; that is correct, not an
error. Lessons appear in it after they are taught, and `git pull` brings each new one. Do
...
```bash
git clone --depth 1 https://github.com/Kepei-Lei102/the-vault-as.git
```
...
no-git path: download the latest `The_Vault_AS_2026.zip` from
<https://github.com/Kepei-Lei102/the-vault-as/releases> and unzip it.

```bash
curl -L -o vault.zip https://github.com/Kepei-Lei102/the-vault-as/releases/latest/download/The_Vault_AS_2026.zip && unzip -q vault.zip && rm vault.zip
```
...
gh repo clone Kepei-Lei102/the-vault-as -- --depth 1
```
...
lost. Open <https://github.com/Kepei-Lei102/the-vault-as/releases>, download
`The_Vault_AS_2026.zip`, and double-click it to unzip. It's about 400 MB, mostly
